...
- Linux는 owner(user) / group / others 로 권한을 나눔
- **권한 표시**: r(읽기)[4] w(쓰기)[2] x(실행)[1]
- **변경**: `chmod`
...
- Remote 해킹 : 해킹하고자 하는 서버에 아이디를 가지고 있지 않을 때, 아이디를 얻고자 시도하는 해킹
- Local 해킹 : 해킹하고자 하는 서버에 일반 계쩡을 가지고 있을 때, 관리자 권한(root)을 얻고자 시도하는 해킹
  - SetUID : 일시적으로 유저의 ID를 변경
  - SetUID와 관련된 공격 기법 : 레이스컨디션, IFS 버그, 링크 버그, 버퍼 오버플로우(BoF), 포맷 스트링 어택 등
